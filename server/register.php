...
$keys = array( "register_password", "register_lot_id", "register_pi_id", "register_area_id" );
foreach( $keys as $key )
...
$stmt->bindValue( 1, $_POST[ "register_lot_id" ], PDO::PARAM_INT );
...
if( $stmt->rowCount() != 0 )
	json_error( 'This combination of pi, lot and area data has already been registered.' );
	
unset($stmt);
unset($query);

// Check that the lot exists
$query = "SELECT lot_id FROM lots WHERE lot_id = ?";
$stmt  = DB::get()->prepare($query);
$stmt->bindValue( 1, $_POST[ "register_lot_id" ], PDO::PARAM_INT );
$stmt->execute();
...
// If there is a row, cannot continue
if( $stmt->rowCount() != 1 )
	json_error( 'This parking lot ID does not exist.' );

// Register the space on the database.
$query2 = "INSERT INTO spaces (space_lot_id, space_pi_id, space_area_code) VALUES (?, ?, ?)";
$stmt2  = DB::get()->prepare( $query2 );
$stmt2->bindValue( 1, $_POST[ "register_lot_id" ], PDO::PARAM_INT );
